...
def read_data(dataset, normalization_method, past_history_factor, forecast_horizon):
    ...
    tmp_data_path = '../data/{}/{}/{}/{}/'.format(dataset, normalization_method, past_history_factor, forecast_horizon)
...
def main_ml(models_ml, datasets, metrics, results_path, parameters_path):
    TRAIN_ML = {
    ...
    for dataset in datasets:
        ...
        for model_name in models_ml:
            ...
            for normalization_method, past_history_factor, forecast_horizon in itertools.product(
                    parameters['normalization_method'],
                    parameters['past_history_factor'],
                    parameters['forecast_horizon']
            ):
                csv_filepath = '{}/{}/results_ml.csv'.format(results_path, dataset)
                ...
                x_train, y_train, x_test, y_test, y_test_denorm, norm_params = read_data(dataset, normalization_method,
                                                                                         past_history_factor, forecast_horizon)
                ...
                for iter_params in itertools.product(*list_parameters_models):
                    ...
                    print('END OF EXPERIMENT -> {}/{}/{}/{}/{}/ML/{}/{}.npy'.format(
                        results_path,
                        dataset,
                        normalization_method,
                        past_history_factor,
...
if __name__ == '__main__':
    models_ml = ['lr', 'rf', 'xgb']
    datasets = ['./data/supply', './data/demand']
    metrics = ['mse', 'rmse', 'mae', 'wape', 'mape']
    parameters_path = './parameters24.json'
    output_path = '../results'
    main_ml(models_ml, datasets, metrics, output_path, parameters_path)
